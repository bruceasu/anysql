...
public class MutipleInvoker implements ModuleInvoker {
    ...
    public MutipleInvoker(PgSqlSQLExecutor executor) {
    ...
    boolean procLOB(String cmdLine) {
        int    i     = TextUtils.getWords(cmdType.getASQLMultiple()[PGSQL_LOB]).size();
    ...
    boolean procLOBEXP(String cmdLine) {
        int    i    = TextUtils.getWords(cmdType.getASQLMultiple()[PGSQL_LOBEXP]).size();
        String str1 = executor.skipWord(cmdLine, i);
        str1 = str1.trim();
        int    j           = 0;
        long   l           = 0L;
    ...
    boolean procLOBLEN(String cmdLine) {
        int    i   = TextUtils.getWords(cmdType.getASQLMultiple()[PGSQL_LOBLEN]).size();
    ...
    boolean procLOBIMP(String cmdLine) {
        long   l1   = 0L;
        int    i    = TextUtils.getWords(cmdType.getASQLMultiple()[PGSQL_LOBIMP]).size();
        String str1 = executor.skipWord(cmdLine, i);
        str1 = str1.trim();
        char[] arrayOfChar = new char[8192];
        byte[] arrayOfByte = new byte[8192];
    ...
    boolean procExplainPlan(String cmdLine) {
        int    i   = TextUtils.getWords(cmdType.getASQLMultiple()[PGSQL_EXPLAINPLAN]).size();
        String str = executor.skipWord(cmdLine, i);
        if (executor.checkNotConnected()) {
            return false;
        }
    ...
    boolean procLoad(String cmdLine) {
        int i = TextUtils.getWords(cmdType.getASQLMultiple()[PGSQL_LOAD])
                         .size();
        String        str1          = executor.skipWord(cmdLine, i);
    ...
    boolean procCross(String cmdLine) {
        int    i   = TextUtils.getWords(cmdType.getASQLMultiple()[PGSQL_CROSS]).size();
        String str = executor.skipWord(cmdLine, i);
        if (executor.checkNotConnected()) {
            return false;
        }
    ...
    private long writeData(PrintStream paramPrintStream,
        ...
        SimpleDateFormat localSimpleDateFormat4 = new SimpleDateFormat(
