...
public abstract class CommandExecutor extends SQLExecutor {
    ...
    public int fetch(ResultSet paramResultSet, DBRowCache paramDBRowCache, int paramInt)
            ...
            for (i = 1; i <= resultSetMetaData.getColumnCount(); i++)
                if (resultSetMetaData.getColumnName(i) != null) {
                    if (paramDBRowCache.findColumn(resultSetMetaData.getColumnName(i)) == 0) {
                        ...
                        for (j = 1; paramDBRowCache.findColumn(resultSetMetaData.getColumnName(i) + "_" + j) != 0; j++)
                            ;
                        paramDBRowCache.addColumn(resultSetMetaData.getColumnName(i) + "_" + j, resultSetMetaData.getColumnType(i));
                    }
                } else {
        ...
        if (paramDBRowCache.getColumnCount() == 0)
            return 0;
        Object[] values;
        for (i = paramDBRowCache.getRowCount();
             (i < paramInt) && (paramResultSet.next());
             i = paramDBRowCache.appendRow(values)) {
            values = new Object[paramDBRowCache.getColumnCount()];
            for (j = 1; j <= paramDBRowCache.getColumnCount(); j++) {
                ...
                switch (paramDBRowCache.getColumnType(j)) {
                    case -1:
                        reader = paramResultSet.getCharacterStream(j);
                        if (reader == null)
                            break;
                        try {
                            m = reader.read(charBuffer2);
                            if (m > 0) value = String.valueOf(charBuffer2, 0, m);
                            reader.close();
                        } catch (IOException e) {
                        }
                    case -4:
                        stream = paramResultSet.getBinaryStream(j);
                        if (stream == null)
                            break;
                        try {
                    ...
                    case 2005:
                        Clob clob = paramResultSet.getClob(j);
                        if (clob == null)
                            break;
                    ...
                    case 2004:
                        Blob blob = paramResultSet.getBlob(j);
                        if (blob == null)
                            break;
                    ...
                    case -3:
                    case -2:
                        stream = paramResultSet.getAsciiStream(j);
                        if (stream == null)
                            break;
                        try {
                    ...
                    case 91:
                        value = paramResultSet.getDate(j);
                        break;
                    case 92:
                        value = paramResultSet.getTime(j);
                        break;
                    case -102:
                    case -101:
                    case 93:
                        value = paramResultSet.getTimestamp(j);
                        break;
                    case -7:
                    case -6:
                    case -5:
                    ...
                    case 8:
                    case 16:
                        value = paramResultSet.getObject(j);
                        break;
                    case -14:
                    case -13:
                    case -10:
                    ...
                    case 2006:
                        ...
                        break;
                    default:
                        value = paramResultSet.getString(j);
                }
                values[(j - 1)] = value;
            }
        }
            ...
            if (i > 0) {
                ...
                for (int j = 0; j < i; j++) {
                    ...
                    if ((arrayOfByte[j] != 13) && (arrayOfByte[j] != 10) && (arrayOfByte[j] != 7)
                            && (arrayOfByte[j] != 9) && (arrayOfByte[j] != 0)
                            && (arrayOfByte[j] < 128) && (arrayOfByte[j] > 0)
                            && (!Character.isISOControl((char) arrayOfByte[j]))) {
    ...
    public final int commandAt(String[] commands, String[] command) {
        ...
        for (i = 0; i < commands.length; i++) {
            j = 0;
            String[] arrayOfString = TextUtils.toStringArray(TextUtils.getWords(commands[i]));
            if (arrayOfString.length > command.length)
                continue;
            k = 1;
            for (j = 0; j < arrayOfString.length; j++) {
                if (arrayOfString[j].equalsIgnoreCase(command[j]))
                    continue;
                k = 0;
                break;
            }
    ...
    public final void debug(String paramString,
        ...
        if (this.debugLevel > 0) {
            ...
            localCommandLog.println("==================== DEBUG L=" + this.debugLevel + " ====================");
            localCommandLog.println("Time: " + new java.util.Date());
            localCommandLog.println(paramString);
            if ((this.debugLevel > 1) && (paramSQLQuery != null) && (paramSQLQuery.getParamNames().length > 0)) {
    ...
    public abstract void doServerMessage()
            throws SQLException;
    ...
    public final SQLStatement prepareScript(Connection paramConnection,
        ...
        localPreparedStatement.setFetchSize(this.fetchSize);
        return new SQLStatement(localPreparedStatement,
    ...
    public final SQLStatement prepareStatement(Connection paramConnection,
            ...
            throws SQLException {
        PreparedStatement localPreparedStatement = null;
        SQLQuery localSQLQuery = SQLConvert.parseSQL(paramString == null ? "" : paramString,
                paramVariableTable);
        debug(localSQLQuery.getDestSQL(), localSQLQuery, paramVariableTable);
        localPreparedStatement = paramConnection.prepareStatement(localSQLQuery.getDestSQL(),
                paramInt1, paramInt2);
        localPreparedStatement.setQueryTimeout(SQL_QUERY_TIMEOUT);
        localPreparedStatement.setFetchSize(this.fetchSize);
    ...
    public final SQLCallable prepareCall(Connection paramConnection,
        ...
        SQLQuery localSQLQuery = SQLConvert.parseCall(paramString == null ? "" : paramString,
                paramVariableTable);
        debug(localSQLQuery.getDestSQL(), localSQLQuery, paramVariableTable);
        localCallableStatement = paramConnection.prepareCall("{ " + localSQLQuery.getDestSQL() + " }",
                paramInt1, paramInt2);
    ...
    public final void run(Command paramCommand)
        ...
        if ((paramCommand.TYPE1 != ASQL_COMMENT) && (paramCommand.TYPE1 != NULL_COMMAND)
            ...
            else if ((paramCommand.TYPE1 == ASQL_SINGLE) || (paramCommand.TYPE1 == ASQL_DBCOMMAND)
                    || (paramCommand.TYPE1 == ASQL_MULTIPLE)) {
                if ((paramCommand.TYPE2 != ASQL_CANCEL) && (!execute(paramCommand)))
                    return;
            } else if (paramCommand.TYPE1 == ASQL_SQLFILE) {
    ...
    public final Command run(CommandReader paramCommandReader)
        ...
        while (true) {
            ...
            if ((localCommand.TYPE1 == ASQL_SINGLE) || (localCommand.TYPE1 == ASQL_DBCOMMAND)
                    || (localCommand.TYPE1 == ASQL_MULTIPLE)) {
                if ((localCommand.TYPE2 == ASQL_CANCEL) || (execute(localCommand)))
                    continue;
                break;
            }
            if (localCommand.TYPE1 == ASQL_SQLFILE) {
    ...
    public final void run()
        ...
        while (true) {
            ...
            command = localCMDType.readCommand(localCommandReader, log, false);
            System.gc();
            if (command.TYPE1 == ASQL_EXIT)
                break;
            if ((command.TYPE1 == ASQL_COMMENT) || (command.TYPE1 == NULL_COMMAND)
                    || (command.TYPE1 == MULTI_COMMENT_START))
                continue;
            ...
            if ((command.TYPE1 == ASQL_SINGLE) || (command.TYPE1 == ASQL_DBCOMMAND)
                    || (command.TYPE1 == ASQL_MULTIPLE)) {
                if ((command.TYPE2 == ASQL_CANCEL) || (execute(command)))
                    continue;
                break;
            }
            if (command.TYPE1 == ASQL_SQLFILE) {
                if (execute(command))
                    continue;
                break;
            }
    ...
    public final void executeSQL(Connection connection,
        ...
        try {
            if ((str.endsWith("/G")) || (str.endsWith("/g")) || (str.endsWith("\\G")) || (str.endsWith("\\g"))) {
                ...
                localSQLStatement = prepareStatement(connection, str.substring(0, str.length() - 2), table);
            } else {
                localSQLStatement = prepareStatement(connection, str, table);
            }
            if (localSQLStatement.stmt == null)
                return;
            localSQLStatement.bind(table);
                ...
                if (bool) {
                    ...
                    if (j >= 0) {
                        ...
                        if (command.TYPE1 == 13) {
                            ...
                            String[] arrayOfString1 = TextUtils.toStringArray(TextUtils.getWords(command.COMMAND.substring(0, Math.min(100, command.COMMAND.length()))));
                            String[] arrayOfString2 = getCommandType().getCommandHint();
                            int k = commandAt(arrayOfString2, arrayOfString1);
                            if (k != -1)
                                log.println(arrayOfString2[k] + " Succeed.");
            ...
            do {
                if (bool) {
                    ...
                    if (j >= 0) {
                        if ((command.TYPE1 == 0) || (command.TYPE1 == 1))
                            log.print(j);
                    } else if ((command.TYPE1 != 0) && (command.TYPE1 != 1) && (this.showComplete))
                        log.println("Procedure executed.");
                }
